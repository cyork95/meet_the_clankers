...
def generate_podcast(mode="daily", holiday=None, categories=None):
    """
    ...
    if holiday:
        print(f"🎉 Holiday Theme: {holiday}")

    # 1. Fetch News
    news = fetch_news(time_window="7d" if mode == "weekly" else "24h", categories=categories)
    ...
    # 2. Generate Script
    result = generate_script(news, mode=mode, holiday_theme=holiday)
...
def run_cli():
    ...
    parser.add_argument("--categories", type=str, default="ai,tech,business,science", help="Comma-separated list of categories")
